...
// Uses array join for efficient string building (avoids creating intermediate strings)
export function formatGuidanceForAnalysis(vendor) {
  const g = GUIDANCE[vendor];
  const parts = [];

  parts.push(`# ${g.name} Prompting Guide\n`);
  parts.push(`## Context Window\n${g.contextWindow || 'Not specified'}\n`);

  parts.push(`## Best Practices`);
  g.bestPractices.forEach((bp, i) => {
    parts.push(`${i + 1}. **${bp.rule}**: ${bp.description}`);
    if (bp.example) {
      parts.push(`   Example: ${bp.example}`);
    }
  });
