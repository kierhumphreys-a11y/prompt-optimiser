// Model guidance data - fetched from Notion with hardcoded fallback
// Source URLs are listed for each vendor
//
// Primary source: Notion database (2d22915b98b080948426eda76c1fdfed)
// Fallback: Hardcoded data below (used when Notion is unavailable)

import { fetchGuidanceFromNotion } from './notion.js';

// Fallback guidance data - used when Notion is unavailable
...
// Cache for guidance data
let guidanceCache = null;

/**
 * Get guidance data from Notion or fallback to hardcoded data
 * This is the primary way to access guidance data
 */
export async function getGuidance() {
  ...
  try {
    const notionGuidance = await fetchGuidanceFromNotion();

    // Only use Notion data if we got at least one vendor
    if (notionGuidance && Object.keys(notionGuidance).length > 0) {
      guidanceCache = notionGuidance;
      return notionGuidance;
    }
  } catch (error) {
    console.warn('Failed to fetch guidance from Notion, using fallback:', error.message);
  }
...
/**
 * Get guidance for a specific vendor
 */
export async function getVendorGuidance(vendor) {
  const guidance = await getGuidance();
  return guidance[vendor] || null;
}
...
/**
 * Clear the guidance cache (forces re-fetch from Notion on next request)
 */
export function clearGuidanceCache() {
  guidanceCache = null;
}

// Legacy export for backwards compatibility
// Note: This uses fallback data synchronously. Prefer getGuidance() for Notion data.
export const GUIDANCE = FALLBACK_GUIDANCE;

// Format guidance for API consumption
function formatGuidanceObject(g) {
  let formatted = `# ${g.name} Prompting Guide\n\n`;
  formatted += `## Context Window\n${g.contextWindow || 'Not specified'}\n\n`;

  formatted += `## Best Practices\n`;
  if (g.bestPractices && Array.isArray(g.bestPractices)) {
  ...
  if (g.formattingPreferences && Array.isArray(g.formattingPreferences)) {
  ...
  if (g.antiPatterns && Array.isArray(g.antiPatterns)) {
...
/**
 * Format guidance for API consumption (async version using Notion data)
 */
export async function formatGuidanceForAnalysis(vendor) {
  const guidance = await getGuidance();
  ...
  return formatGuidanceObject(g);
}

/**
 * Format guidance for API consumption (sync version using fallback data)
 * @deprecated Use formatGuidanceForAnalysis() instead
 */
export function formatGuidanceForAnalysisSync(vendor) {
  const g = FALLBACK_GUIDANCE[vendor];
